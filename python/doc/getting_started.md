...
## A Developer Guide for Hyperledger Indy-SDK `getting_started.py` Script
...
- [Getting Started with Libindy](#getting-started-with-libindy)
  ...
  - [Step 0. Connect to the Indy Nodes Pool](#step-0-connect-to-the-indy-nodes-pool)
  - [Step 0.0: Getting Trust Anchor Credentials for Sovrin Steward](#step-00-getting-trust-anchor-credentials-for-sovrin-steward)
    - [Step 0.1: Getting Trust Anchor Credentials - Sovrin Steward](#step-01-getting-trust-anchor-credentials---sovrin-steward)
  - [Step 1.0: Getting Trust Anchor Credentials for Sovrin Steward, Government, Faber College, Acme Corp, and Thrift Bank](#step-10-getting-trust-anchor-credentials-for-sovrin-steward-government-faber-college-acme-corp-and-thrift-bank)
    - [Step 1.1: Onboarding the Government](#step-11-onboarding-the-government)
    - [Step 1.2: Onboarding Faber College](#step-12-onboarding-faber-college)
        - [Connecting the Establishment](#connecting-the-establishment)
        - [Getting Verinym](#getting-verinym)
    - [Step 1.3: Onboarding Acme Corp](#step-13-onboarding-acme-corp)
    - [Step 1.4: Onboarding Thrift Bank](#step-14-onboarding-thrift-bank)
  - [Step 2.0: Credential Schemas Setup](#step-20-credential-schemas-setup)
    - [Step 2.1 Create Job Certificate Schema - Government](#step-21-create-job-certificate-schema---government)
    - [Step 2.2 Create Transcript Schema - Government](#step-22-create-transcript-schema---government)
  - [Credential Definition Setup](#credential-definition-setup)
    - [Step 2.3 Credential Definition Setup - Faber College](#step-23-credential-definition-setup---faber-college)
    - [Step 2.4 Credential Defintion Setup - Acme Corp](#step-24-credential-defintion-setup---acme-corp)
  - [Step 3.0 Alice Gets a Transcript from Faber College](#step-30-alice-gets-a-transcript-from-faber-college)
    - [Step 3.1 Getting the Transcript - Onboarding - Faber College](#step-31-getting-the-transcript---onboarding---faber-college)
    - [Step 3.2 Getting the Transcript - Getting the Transcript Credential - Faber College](#step-32-getting-the-transcript---getting-the-transcript-credential---faber-college)
  - [Step 4.0 Apply for a Job at Acme Corp](#step-40-apply-for-a-job-at-acme-corp)
    - [Step 4.1 Applying for a Job at Acme Corp - Onboarding - Alice](#step-41-applying-for-a-job-at-acme-corp---onboarding---alice)
    - [Step 4.2 Applying for a Job at Acme Corp - Transcript Proving](#step-42-applying-for-a-job-at-acme-corp---transcript-proving)

## What are Hyperledger Indy and `libindy` and Why They Matter

Hyperledger Indy (Indy) provides a software ecosystem for private, secure, and powerful identity, and `libindy` enables clients for it. Indy puts people — not the organizations that traditionally centralize identity — in charge of decisions about their own privacy and disclosure. This enables all kinds of rich innovation: connection contracts, revocation, novel payment workflows, asset and document management features, creative forms of escrow, curated reputation, integrations with other cool technologies, and so on.
...
Becoming a **Trust Anchor** requires contacting a person or organization who already has the **Trust Anchor** role on the ledger. For the sake of the demo, in our empty test ledger we have only NYMs with the **Sovrin Steward** role, but all **Sovrin Stewards** are automatically **Trust Anchors**.
...
TODO - Agent communications is simulated.

## Step 0. Connect to the Indy Nodes Pool
...
The code block below contains each of these items. Note how the comments denote that this is the code for the "Sovrin Steward Agent."
...
## Step 0.0: Getting Trust Anchor Credentials for Sovrin Steward

The first code block will contain the code of the **Sovrin Steward's** agent.

### Step 0.1: Getting Trust Anchor Credentials - Sovrin Steward

**Next, the Sovrin Steward's agent should get the ownership for the DID that has corresponding NYM transactions with the Sovrin Steward role on the ledger.**

![Getting Trust Anchor Credentials - Sovrin Steward](images/Indy-SDK-Getting-Started-0.1.png)

The test ledger we use was pre-configured to store some known **Sovrin Steward** NYMs. Also we know **seed** values for the random number generator that were used to generate keys for these NYMs. These **seed** values allow us to restore signing keys for these DIDs on the **Sovrin Steward's** agent side and as a result get the DID ownership.

Libindy has a concept of the **Wallet**. The wallet is secure storage for crypto materials like DIDs, keys, etc... To store the **Sovrin Steward's** DID and corresponding signkey, the agent should create a named wallet first by calling ``wallet.create_wallet``. After this the named wallet can be opened by calling ``wallet.open_wallet``. This call returns the wallet handle that can be used to reference this opened wallet in future `libindy` calls.
...
```python
  # Sovrin Steward Agent
  steward = {
      'name': "Sovrin Sovrin Steward",
      'wallet_config': json.dumps({'id': 'sovrin_steward_wallet'}),
      'wallet_credentials': json.dumps({'key': 'steward_wallet_key'}),
      'pool': pool_['handle'],
      'seed': '000000000000000000000000Sovrin Steward1'
  }
...
### Step 1.1: Onboarding the Government

TODO

![Getting Trust Anchor Credentials - Government](images/Indy-SDK-Getting-Started-1.1.png)

TODO

### Step 1.2: Onboarding Faber College

TODO

![Getting Trust Anchor Credentials - Faber College](images/Indy-SDK-Getting-Started-1.2.png)

TODO

##### Connecting the Establishment
Let's look the process of connection establishment between **Sovrin Steward** and **Faber College**.

1. **Faber College** and **Sovrin Steward** contact in some way to initiate onboarding process.
   It can be filling the form on web site or a phone call.

2. **Sovrin Steward** creates a new DID record in the wallet by calling ``did.create_and_store_my_did`` that he will use for secure interactions only with **Faber College**.
    ```python
    # Sovrin Steward Agent
    (steward['did_for_faber'], steward['key_for_faber']) = await did.create_and_store_my_did(steward['wallet'], "{}")
    ```

3. **Sovrin Steward** sends the corresponding `NYM` transaction to the Ledger by consistently calling ``ledger.build_nym_request`` to build the NYM request and ``ledger.sign_and_submit_request`` to send the created request.
    ```python
    # Sovrin Steward Agent
...
4. **Sovrin Steward** creates the connection request which contains the created `DID` and `Nonce`.
...
5. **Sovrin Steward** sends the connection request to **Faber College**.

6. **Faber College** accepts the connection request from **Sovrin Steward**.

7. **Faber College** creates a wallet if it does not exist yet.
    ```python
    # Faber Agent
...
8. **Faber College** creates a new DID record in its wallet by calling ``did.create_and_store_my_did`` that it will use only for secure interactions with the **Sovrin Steward**.
    ```python
    # Faber Agent
    (faber['did_for_steward'], faber['key_for_steward']) = await did.create_and_store_my_did(faber['wallet'], "{}")
    ```

9. **Faber College** creates the connection response which contains the created `DID`, `Verkey` and `Nonce` from the received connection request.
    ```python
    # Faber Agent
...
10. **Faber College** asks the ledger for the Verification key of the **Sovrin Steward's** DID by calling ``did.key_for_did``.
    ```python
    # Faber Agent
    faber['steward_key_for_faber'] = await did.key_for_did(faber['pool'], faber['wallet'], connection_request['did'])
    ```

11. **Faber College** anonymously encrypts the connection response by calling ``crypto.anon_crypt`` with the **Sovrin Steward** verkey.
...
12. **Faber College** sends the anonymously encrypted connection response to the **Sovrin Steward**.

13. **Sovrin Steward** anonymously decrypts the connection response by calling ``crypto.anon_decrypt``.
    ```python
    # Sovrin Steward Agent
...
14. **Sovrin Steward** authenticates **Faber College** by the comparison of Nonce.
    ```python
    # Sovrin Steward Agent
    assert connection_request['nonce'] == decrypted_connection_response['nonce']
    ```

15. **Sovrin Steward** sends the `NYM` transaction for **Faber's** DID to the Ledger.
Please note that despite the fact that the Sovrin Steward is the sender of this transaction the owner of the DID will be Faber as it uses the verkey as provided by Faber.
    ```python        
    # Sovrin Steward Agent
...
At this point **Faber College** is connected to the **Sovrin Steward** and can interact in a secure peer-to-peer way. **Faber College** can trust the response is from **Sovrin Steward** because:

* it connects to the current endpoint
* no replay - attack is possible, due to her random challenge
* it knows the verification key used to verify **Sovrin Steward** digital signature is the correct one because it just confirmed it on the ledger
...
It is important to understand that earlier created **Faber College** DID is not, in and of itself, the same thing as self-sovereign identity. This DID must be used only for secure interaction with the **Sovrin Steward**.
After the connection is established **Faber College** must create new DID record that he will use as Verinym in the Ledger.

1. **Faber College** creates a new DID in its wallet by calling ``did.create_and_store_my_did``.
    ```python        
    # Faber Agent
    (faber['did'], faber['key']) = await did.create_and_store_my_did(faber['wallet'], "{}")
    ```

2. **Faber College** prepares the message that will contain the created DID and verkey.
    ```python        
    # Faber Agent
...
4. **Faber College** sends the encrypted message to the **Sovrin Steward**.

5. **Sovrin Steward** decrypts the received message by calling ``crypto.auth_decrypt``.
    ```python        
    # Sovrin Steward Agent    
...
6. **Sovrin Steward** asks the ledger for the Verification key of **Faber's** DID by calling ``did.key_for_did``.
    ```python        
    # Sovrin Steward Agent    
    steward['faber_key_for_steward'] = await did.key_for_did(steward['pool'], steward['wallet'], ['faber_did_for_steward'])
    ```

7. **Sovrin Steward** authenticates **Faber College** by comparison of the Message Sender Verkey and the **Faber College** Verkey received from the Ledger.
    ```python        
    # Sovrin Steward Agent    
    assert sender_verkey == steward['faber_key_for_steward']
    ```

8. **Sovrin Steward** sends the corresponded NYM transaction to the Ledger with `TRUST ANCHOR` role.
Please note that despite the fact that the Sovrin Steward is the sender of this transaction the owner of DID will be Faber as it uses Verkey provided by Faber.
    ```python    
    # Sovrin Steward Agent
    ...
    await ledger.sign_and_submit_request(steward['pool'], steward['wallet'], steward['did'], nym_request)
    ```
At this point **Faber College** has a DID related to his identity in the Ledger.

**Acme Corp**, **Thrift Bank**, and **Government** must pass the same Onboarding process connection establishment with **Sovrin Steward**.

### Step 1.3: Onboarding Acme Corp

TODO

![Getting Trust Anchor Credentials - Acme Corp](images/Indy-SDK-Getting-Started-1.3.png)

TODO

### Step 1.4: Onboarding Thrift Bank

TODO

![Getting Trust Anchor Credentials - Thrift Bank](images/Indy-SDK-Getting-Started-1.4.png)

TODO

## Step 2.0: Credential Schemas Setup
...
### Step 2.1 Create Job Certificate Schema - Government

TODO

![Create Job Certificate Schema - Government](images/Indy-SDK-Getting-Started-2.1.png)

Here is where the **Government** creates and publishes the **Job-Certificate** Credential Schema to the Ledger:
```python    
  # Government Agent
    job_certificate = {
...
### Step 2.2 Create Transcript Schema - Government

TODO

![Create Transcript Schema - Government](images/Indy-SDK-Getting-Started-2.2.png)

Similarly, **Government** creates and publishes the **Transcript** Credential Schema to the Ledger:
...
At this point we have the **Job-Certificate** and the **Transcript** Credential Schemas published by **Government** to the Ledger.
...
### Step 2.3 Credential Definition Setup - Faber College

Here **Faber College** creates and publishes a Credential Definition for the known **Transcript** Credential Schema to the Ledger.

![Credential Definition Setup - Faber College](images/Indy-SDK-Getting-Started-2.3.png)
...
### Step 2.4 Credential Defintion Setup - Acme Corp

TODO

![Credential Defintion Setup - Acme Corp](images/Indy-SDK-Getting-Started-2.4.png)

The same way **Acme Corp** creates and publishes a **Credential Definition** for the known **Job-Certificate** Credential Schema to the Ledger.
```python
  # Acme Agent
...
At this point we have a **Credential Definition** for the **Job-Certificate** Credential Schema published by **Acme Corp** and a
 **Credential Definition** for the **Transcript** Credential Schema published by **Faber College**.

## Step 3.0 Alice Gets a Transcript from Faber College
...
As we mentioned in [About Alice](#about-alice), **Alice** graduated from **Faber College**. TODO


![Getting the Transcript from Faber College](images/Indy-SDK-Getting-Started-3.0.png)

### Step 3.1 Getting the Transcript - Onboarding - Faber College
...
### Step 3.2 Getting the Transcript - Getting the Transcript Credential - Faber College
...
**Faber College** prepares both raw and encoded values for each attribute in the **Transcript** Credential Schema.
**Faber College** creates the **Transcript** Credential for Alice.
```python
  # Faber Agent
...
## Step 4.0 Apply for a Job at Acme Corp
...
The process of the connection establishment is the same as when Faber was accepting the Sovrin Steward connection request.

![Apply for a Job at Acme Corp](images/Indy-SDK-Getting-Started-4.0.png)

### Step 4.1 Applying for a Job at Acme Corp - Onboarding - Alice
...
When **Acme Corp** inspects the received Proof he will see following structure:
```
  # Acme Agent
  {
      'requested_proof': {
...
### Step 4.2 Applying for a Job at Acme Corp - Transcript Proving

TODO

**Acme Corp** got all the requested attributes. Now **Acme Corp** wants to check the Validity Proof.
To do it **Acme Corp** first must get every Credential Schema and corresponding Credential Definition for each identifier presented in the Proof, the same way that Alice did it.
Now **Acme Corp** has everything to check **Job-Application** Proof from Alice.
 ```python
  # Acme Agent
...
**Acme Corp** creates new Credential Offer for Alice.
```python
  # Acme Agent
...
### Step 4.3 Applying for a Job at Acme Corp - Getting Job Certificate Credential

TODO

## Step 5.0 Alice applies for a Loan with Thrift Bank
...
![Alice applies for a Loan with Thrift Bank](images/Indy-SDK-Getting-Started-5.0.png)

### Step 5.1 Alice applies for a Loan with Thrift Bank - Onboarding
...
### Step 5.2 Alice applies for a Loan with Thrift Bank - Job Certificate Proving
...
### Step 5.3 Alice applies for a Loan with Thrift Bank - Transcript and Job Certificate Proving

TODO

When **Thrift Bank** inspects the received Proof he will see following structure:
```
  # Thrift Agent
  {
      'requested_proof': {
...
### Step 6.0 Close and Cleanup

TODO


![Close and Cleanup](images/Indy-SDK-Getting-Started-6.0.png)
